// components/sales/pos/edit/edit-mobile-cart-sheet.tsx
"use client";
...
import { EditCartPanel } from "./edit-cart-panel";
import { EditSaleOptionsPanelProps, EditSaleOptionsPanel } from "./edit-sale-options-panel";
...
type Props = {
  ...
  cart: CartItem[];
  total: number;
  cartProps: React.ComponentProps<typeof EditCartPanel>;
  optionsProps: EditSaleOptionsPanelProps;
};

export function EditMobileCartSheet({
  open,
  onOpenChange,
  cart,
  total,
  ...
  useEffect(() => {
    ...
    const handler = (e: MouseEvent) => {
      ...
      const inPortal = (target as Element).closest?.(
        "[data-radix-popper-content-wrapper], [data-radix-portal], [role='listbox'], [role='dialog']"
      );
      if (inPortal) return;
      ...
      >
        ...
        >
          <div className="px-4 py-3 space-y-3 pb-10">
            ...
            {cart.length > 0 && (
              <EditSaleOptionsPanel {...optionsProps} />
            )}
          </div>
        </div>
      </div>
